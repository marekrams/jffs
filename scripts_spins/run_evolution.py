...
from operators import HXXZ, sumLn2, Ln, momentum_total, momentum_n, measure_energy_per_site, measure_T_per_site, measure_Ln, measure_currents
...
def folder_evol(g, m, L0, a, N, v, Q, D):
    path = Path(f"./results/{g=:0.4f}/{m=:0.4f}/{L0=:d}/{N=}/{a=:0.4f}/{v=:0.4f}/{Q=:0.4f}/{D=}/")
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def run_evol(g, m, L0, a, N, D0, v, Q, dt, snapshots, snapshots_states):
    ops = yastn.operators.Spin12(sym='U1')
    #
    try:
    ...
    e0 = a * g * g / 2
    t = 0
    engs_gs = measure_energy_per_site(psi, a, g, m, t, L0, v, Q, ops)

    folder = folder_evol(g, m, L0, a, N, v, Q, D0)
    ...
    for step in mps.tdvp_(psi, Ht, times, dt=dt,
        ...
        engs = measure_energy_per_site(psi, a, g, m, step.tf, L0, v, Q, ops)
        ents = psi.get_entropy()
        T00, T11, T01 = measure_T_per_site(psi, a, g, m, L0, ops)
        Ln = measure_Ln(psi, L0, ops)
...
if __name__ == "__main__":
    ...
    #         job = run_gs.remote(g, m, L0, a, N, D0)
    #         refs.append(job)
    # ray.get(refs)
    ...
    for m in [0 * g, 0.1 * g, 0.318 * g, 1 * g]:
        for N, a in [(100, 1.0), (200, 0.5), (400, 0.25)]:
            job = run_evol.remote(g, m, L0, a, N, D0, 1, 1, 1/8, 100, 20)
            refs.append(job)
    ray.get(refs)
